...
if($get_user_id == 0){
	...
	$userInfo = new user($get_user_id);
	if($userInfo->last_name == "")
		$userInfo = $userInfo->username;
	else
		$userInfo = $userInfo->last_name." ".$userInfo->name;
...
?>										
										...
										<?php
											...
											if($user_right == 0)
												$reponse = $bdd->query("SELECT * FROM message WHERE (message_from = '$get_user_id' AND user = 'user' AND message_to = 0) OR (message_to = '$get_user_id' AND user = 'admin' AND message_from = 0) ORDER BY date_creation");
	   										else
	   											$reponse = $bdd->query("SELECT * FROM message WHERE (message_from = '$user_id' AND user = 'user' AND message_to = 0) OR (message_to = '$user_id' AND user = 'admin' AND message_from = 0) ORDER BY date_creation");
	   										//echo "SELECT * FROM message WHERE (message_from = '$get_user_id' AND user = 'user' AND message_to = 0) OR (message_to = '$get_user_id' AND user = 'admin' AND message_from = 0) ORDER BY date_creation";
											...
											<div class="chat_submit">
												<div class="input-group">
													...
													<span class="input-group-btn">
														<button class="btn btn-primary btn-lg send" type="button" data-id='<?php echo $get_user_id; ?>' data-type='user'><span class="icon-comments"></span></button>
													</span>
												</div>
											</div>
										</div>
	...
	jQuery(document).ready(function($){
			...
			function time_passe(timestamp){
				...
				function refreshMessage(){
					var target_id = $('.send').data('id');
					var message_type = $('.send').data('type');
					$.post('php/admin.messageGet.php',{target_id:target_id,message_type:message_type},function(result){
...
<?php if($user_right == 0){
		echo"$.post('php/script.sendMessage.php?action=AdminToUser',{user:'admin',message_from:0,message_to:target_id,type:'text',content:message},function(result){";
		echo"$('.input-lg').val('');";
		...
		echo"$.post('php/script.sendMessage.php?action=UserToAdmin',{user:'user',message_from:user_id,message_to:0,type:'text',content:message},function(result){";
		echo"$('.input-lg').val('');";
		echo"$('.chat_messages').append(afficheMessage(result[0].id,result[0].message_from,result[0].message_to,result[0].content,result[0].date,result[0].message_timestamp,result[0].type,result[0].user,result[0].photo,result[0].link_id,result[0].link_photo));";			
		echo"if(result.length != 0){";
		echo"var objDiv = document.getElementById('chat_messages');";
