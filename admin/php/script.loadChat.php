...
if($target_id == 0){
	$affiche = "Home-Saigon Staff";
}else{
	$userInfo = new user($target_id);
	if($userInfo->last_name != "")
		$affiche = $userInfo->last_name." ".$userInfo->name;
	else
		$affiche = $userInfo->username;
}


?>										
										<div class="panel-heading">
											<h4 class="panel-title pull-left">Messages with: <?php echo $affiche; ?></h4>
										...
										<?php
											...
											if($target_id == 0){
												$reponse = $bdd->query("SELECT * FROM message WHERE (message_from = '$client_id' AND user = 'client' AND message_to ='0') OR (message_to = '$client_id' AND user = 'admin' AND message_from = 0 ) ORDER BY date_creation");
	   											...
	   											$reponse = $bdd->query("SELECT * FROM message WHERE (message_from = '$client_id' AND user = 'client' AND message_to = '$target_id') OR (message_to = '$client_id' AND user = 'user' AND message_from ='$target_id') ORDER BY date_creation");
	   										...
	   										foreach ($reponse as $key => $value) {
	   											...
	   											if($message->user == 'admin' || $message->user == 'user'){
	   												if($value['message_from'] != 0){
	   													$userData = new user($value['message_from']);
	   													$body.="<div class='chat_message ' data-id='".$value['id']."'>";
	   													if($userData->photo != "")
	   														$body.="<img alt='' src='".$userData->photo."' class='img-thumbnail'>";
	   													else
	   												...
	   												$clientData = new client($value['message_from']);
	   													$body.="<div class='chat_message ch_right' data-id='".$value['id']."'>";
	   													if($clientData->photo != "")
	   														$body.="<img alt='' src='".$clientData->photo."' class='img-thumbnail'>";
	   													else
	   														$body.="<img alt='' src='empty-profile.png' class='img-thumbnail'>";
	   											}
											...
											<div class="chat_submit">
												<div class="input-group">
													...
													<span class="input-group-btn">
														<button class="btn btn-primary btn-lg send" type="button" data-id='<?php echo $target_id; ?>' data-type='user'><span class="icon-comments"></span></button>
													</span>
												</div>
											</div>
										</div>
	...
	jQuery(document).ready(function($){
			...
			function time_passe(timestamp){
				...
				function refreshMessage(){
					...
					$.post('php/script.messageGet.php',{user_id:user_id,client_id:client_id},function(result){
				...
				$('.send').on('click',function(){
			 		...
			 		$.post('php/script.sendMessage.php?action=ClientToUser',{user:'client',message_from:client_id,message_to:target_id,type:'text',content:message},function(result){
			 			 $('.input-lg').val('');
			 			$('.chat_messages').append(afficheMessage(result[0].id,result[0].message_from,result[0].message_to,result[0].content,result[0].date,result[0].message_timestamp,result[0].type,result[0].user,result[0].photo,result[0].link_id,result[0].link_photo));			
			 			if(result.length != 0){
