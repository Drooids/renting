	...
	public function getObject($donnees)
	  {
	 	...
	 	global $bdd;
	   	$reponse = $bdd->query("SELECT * FROM emailSent where id = '$donnees'");
	  ...
	  public function create()
	  {
	    ...
	    $bdd->exec("INSERT INTO emailSent (name,date_creation)
								 VALUES ('New list','$date')");

	    $reponse = $bdd->query("SELECT id FROM emailSent ORDER BY id DESC LIMIT 1");
	   	$id = $reponse->fetchAll();
	   	$id = $id[0]['id'];
	  ...
	  public function save_one_data($name,$data)
	  {
		...
		$this->AddVar($name,$data);
	  	if($bdd->exec("UPDATE emailSent SET ".$name." = '$data' WHERE id ='$id'")){
	  		return true;
	  	}else{
	  		return false;
	  	}
