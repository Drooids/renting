...
if($client_id == 0){
	$clientInfo = "Home-Saigon Staff";
}else{
	$clientInfo = new client($client_id);
	$clientInfo = $clientInfo->last_name." ".$clientInfo->name;
}


?>										
										<div class="panel-heading">
											<h4 class="panel-title pull-left">Messages with: <?php echo $clientInfo; ?></h4>
										...
										<?php
											...
											if($user_right == 0){
												$reponse = $bdd->query("SELECT * FROM message WHERE (message_from = '$client_id' AND user = 'client' AND message_to = 0) OR (message_to = '$client_id' AND user = 'admin' AND message_from = 0) ORDER BY date_creation");
	   										}else{
	   											$reponse = $bdd->query("SELECT * FROM message WHERE (message_from = '$client_id' AND user = 'client' AND message_to = '$user_id') OR (message_to = '$client_id' AND user = 'user' AND message_from ='$user_id') ORDER BY date_creation");
	   										...
	   										//echo "SELECT * FROM message WHERE (message_from = '$client_id' AND user = 'client' AND message_to = 0) OR (message_to = '$client_id' AND user = 'admin' AND message_from = 0) ORDER BY date_creation";
	   									...
	   									//	echo "number of data: ".count($reponse);
	   										...
	   										foreach ($reponse as $key => $value) {
	   											...
	   											if($message->user == 'user' || $message->user == 'admin'){
	   											...
	   											switch ($value['type']) {
	   												...
	   												case 'devis':
	   													...
	   													$verif =$value['link_id'];
	   													$verif = $bdd->query("SELECT id_portfolio FROM portfolio WHERE id_portfolio ='$verif'");
                                        				...
                                        				if(count($verif) == 1){
															...
															if($portfolio->actif_portfolio == 0){
																$body.=$value['content'];
																$body.="</br><a href='#' data-link='".$value['link_id']."' class='property'> See the property </a>";
															}else{
																$body.="This property has been removed by the owner. The property is not available.";
															}
															$body.="</div>";
															$body.="</div>";
                                        				}
											...
											<div class="chat_submit">
												<div class="input-group">
													...
													<span class="input-group-btn">
														<button class="btn btn-primary btn-lg send" type="button" data-id='<?php echo $client_id; ?>' data-type='client'><span class="icon-comments"></span></button>
													</span>
												</div>
											</div>
										</div>
	...
	jQuery(document).ready(function($){
			...
			function time_passe(timestamp){
				...
				function afficheMessage(id,from,to,content,date,timestamp,type,user,photo,link_id,link_photo){
					var text = "";
					var userRight = "<?php echo $user_right; ?>";

					if(user == 'admin' && userRight == 0 )
						text +="<div class='chat_message ch_right' data-id='"+id+"'>";
					else if(user == 'user' && userRight > 0)
						text +="<div class='chat_message ch_right' data-id='"+id+"'>";
					else
				...
				function refreshMessage(){
					var client_id = $('.send').data('id');
					var message_type = $('.send').data('type');
					$.post('php/admin.messageGet.php',{target_id:client_id,message_type:message_type},function(result){
...
<?php if($user_right == 0){
		echo"$.post('php/script.sendMessage.php?action=AdminToClient',{user:'admin',message_from:0,message_to:client_id,type:'text',content:message},function(result){";
		echo"$('.input-lg').val('');";
		echo"$('.chat_messages').append(afficheMessage(result[0].id,result[0].message_from,result[0].message_to,result[0].content,result[0].date,result[0].message_timestamp,result[0].type,result[0].user,result[0].photo,result[0].link_id,result[0].link_photo));";			
		echo"if(result.length != 0){";
		echo"var objDiv = document.getElementById('chat_messages');";
		echo"objDiv.scrollTop = objDiv.scrollHeight;";
		echo"}";
		echo"});";
}else{
		echo"$.post('php/script.sendMessage.php?action=UserToClient',{user:'user',message_from:user_id,message_to:client_id,type:'text',content:message},function(result){";
		echo"$('.input-lg').val('');";
		echo"$('.chat_messages').append(afficheMessage(result[0].id,result[0].message_from,result[0].message_to,result[0].content,result[0].date,result[0].message_timestamp,result[0].type,result[0].user,result[0].photo,result[0].link_id,result[0].link_photo));";			
		echo"if(result.length != 0){";
		echo"var objDiv = document.getElementById('chat_messages');";
